...
class MetaPath2VecRecommender(Recommender):
    def __init__(self, config):
        super().__init__(config)

    def fit(self, adj_data, w2v_embeddings, metapath, val_data, test_data, logger):
        # Select evaluation function based on dataset
        ...
        # Initialize embeddings with Word2Vec if provided
        if w2v_embeddings:
            with torch.no_grad():
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            print(f"Epoch {epoch} Total Loss: {loss:.4f}, Recall@{max(self.k)}: {recall_k}")
